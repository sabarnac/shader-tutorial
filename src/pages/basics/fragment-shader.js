    ...
    <Layout>
      ...
      <Content>
        ...
        <p>
          ...
          fragment shader operates on a "fragment" of an object and tells what
          the color of that fragment is supposed to be.
        </p>
        <p>
          Since it is executed per fragment on all fragments generated by the
          GPU pipeline, any operation that requires modification of the color of
          the fragment (like brightening due to lights or darkening due to
          shadows) can be done through the fragment shader.
        </p>
        ...
        <p>
          A pixel can consist of multiple fragments, because, depending upon
          what area of the primitive the pixel covers, there can be multiple
          values present within the pixel, which have to either be combined, or
          just one selected at random.
        </p>
        <p>
          As an example, take a circle that is to be rendered on a screen of
          size 8x8 pixels (total of 64 pixels), as shown by the image below:
        </p>
        ...
        <p>
          ...
          circle and the background. This requires a decision needs to be made
          as to what the final color of the pixel should be.
        </p>
        <p>
        ...
        <p>
          ...
          since two of these fragments would have the color red, and the other
          two would have the color white:
        </p>
        <p className="util text-center">
          <img
        ...
        <p>
          ...
          reality compared to the first result. If you're not sure how, let's
          see how these images look at 30px width and height.
        </p>
        <p className="util text-center">
          <img
        ...
        <p>
          At a much smaller scale, the second result looks considerably more
          like a circle than the first result. This is how certain anti-aliasing
          methods works.
        </p>
        <p>
          ...
          fragments if some of them are not opaque (a fragment from a
          translucent glass over an object).
        </p>
        <p>
          Do note that in DirectX, fragments are called pixels (and by
          extension, fragment shaders are called pixel shaders), but that name
          isn't technically accurate.
        </p>
        <h3>An example - The triangle returns</h3>
        <p>
          Let's go back to our standard triangle example. Previously, we only
          had the edges of the triangle drawn to provide an explanation on how
          vertex shaders work. This time we'll be coloring the entire triangle.
        </p>
        ...
        />
        <p>
          Looking at the code, you'll see the fragment shader is extremely
          simple, it just generates a <code>vec4</code> of{" "}
          <code>1.0, 0.0, 0.0, 0.0</code>, which corresponds to R, G, B, and A
          values respectively. The color is then assigned to the special output
          variable defined in WebGL called <code>gl_FragColor</code> (WebGL
          Fragment Color)
        </p>
        <p>
        ...
        <p>
          ...
          fragments of the lines that joined the first and second vertex, then
          the line connecting the second and third vertex, and (since it was
          defined to be a loop), the line connecting third and first vertex.
        </p>
        <p>
          In this case, since WebGL is told the vertices belong to a triangle,
          the 3 vertices are taken and used to create a triangle, and that
          triangle is then split into fragments, with each fragment then colored
          by the fragment shader.
        </p>
        <h3>Another example - A triangular color wheel</h3>
        <p>
          What if we wanted to define our own color values that should be used
          to color the pixels? Just like with the vertex shader, we can pass our
          own needed values to the fragment shader.
        </p>
        <p>
        ...
        <p>
          One very important note is that you can only define the color values
          of each vertex, and not each fragment or pixel, since their number is
          an unknown (you won't know how many fragments may be generated, or
          pixels that the object may cover).
        </p>
        <p>
        ...
        <p>
          When we set the colors of the vertices, and then the object/primitive
          is set into fragments, the fragments covering the vertices
          automatically get the color of that vertex, since that is what that
          fragment represents.
        </p>
        <p>
          The color of other fragments is determined by checking its position
          and distance relative to every vertex, and interpolating what its
          color value should be based upon the distance.
        </p>
        <p>
        ...
        <p>
          The vertex shader then passes this on to the fragment shader through
          the <code>vec3</code> varying named <code>color</code>.
        </p>
        <p>
        ...
        <p>
          As a result, when we pass the color of the vertices down to the
          fragment shaders, the value is interpolated based on the position of
          the fragment, and then passed to the fragment.
        </p>
        <p>
        ...
        <h4>How it works</h4>
        <p>
          Similar to how we did the rotating triangle in the previous chapter,
          we determine how much to shift the color by relative to the current
          timestamp.
        </p>
        <p>
          This calculation is, similar to the rotating triangle, also done on
          the CPU since it only needs to be done once, and can save redundant
          calculations done repeatedly on the GPU.
        </p>
        <p>
          By subtracting the calculated color shift from the interpolated
          fragment color, we can have the color of the triangle oscillate from
          pure black, to the standard triangle color wheel, to pure white, and
          back again.
        </p>
        <p>
        ...
        <p>
          <em>
            ...
            <code>color - colorShift</code>) and makes sure it doesn't go
            outside of a certain range (<code>0.0</code> and <code>1.0</code>),
            "clamping" it to either end depending on if it is too large or too
            small. It also works with both vector (<code>vec2</code>,{" "}
            <code>vec3</code>, <code>vec4</code>) and scalar values (
        ...
        <ul>
          ...
          <li>
            The fragment shader requires certain values in order to determine
            what the final color of the fragment should be.
            <ul>
              <li>
                If this data is passed through the vertex shader, its actual
                value will be interpolated by the GPU based on the distance of
                the fragment from each vertex of the primitive.
              </li>
              <li>
