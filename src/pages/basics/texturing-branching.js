  ...
  return (
    <Layout>
      ...
      <Content>
        ...
        <p>
          So far we've passed direct color information of vertices to the
          fragment shader, which was then interpolated for each fragment and
          then set as the pixel color.
        </p>
        <p>
          As learnt in the{" "}
          <Link to="/basics/fragment-shader">previous chapter</Link>, color
          information can only be mapped to the vertices of an object, with
          color values of parts of the object between the vertices being
          interpolated.
        </p>
        <p>
        ...
        <p>
          Let's first learn about how a texture is overlayed onto an object.
        </p>
        <h3>UV Mapping</h3>
        <p>
        ...
        <p>
          ...
          fragment shader, with the GPU interpolating the coordinates as need
          for each fragment.
        </p>
        <p>
        ...
        <p>
          ...
          through the <code>vertexUv</code> attribute, which is then passed to
          the fragment shader <code>uv</code>, allowing the GPU to interpolate
        ...
        <p>
          <code>textureSampler</code> is defined as <code>sampler2D</code>{" "}
        ...
        <h4>How it works</h4>
        <p>
          Similar to how the pulsing triangle was made in the last example of
          the <Link to="/basics/fragment-shader">previous chapter</Link>, a{" "}
          <code>colorShift</code> value is subtracted from the color values
          retrieved from the texture (except for alpha), and then set as the
          final color of the fragment.
        </p>
        <p>
          One unique change is the white edges of the cube, which do not pulse
          and always stay constant.
        </p>
        <GlslCodeHighlight
        ...
        />
        <p>
          In the code, a new function <code>getColorShiftFactor</code> accepts
          an RGB <code>color</code> variable, and using a formula, decides
          whether the color shift should be factored by 0 or 1.
        </p>
        <p>
        ...
        <p>
          ...
          affecting the texture color value as it normally would when setting
          the final fragment color value.
        </p>
        ...
        <ul>
          ...
          <li>
            Ceil the difference at or above it (ex: it would change the number
            2.1 to 3).
          </li>
          <li>
        ...
        <p>
          When a color is white, it's RGB components would all be at their
          highest value, which is 1. As a result, their sum would be 3. When
          this sum is subtracted from the value 3, any color value that is not
          white would result in a difference greater than 0.
        </p>
        <p>
          This will result in the ceiling value of any color that is not white
          be least 1 or higher, which is then clamped to 1.
        </p>
        <p>
        ...
        <p>
          A question that might come up is why not just use conditions to check
          if a color is white, and depending on the result, just return 0 or 1
          accordingly. The reason this is not done is because using branches (if
          conditions) is generally discouraged in shader code.
        </p>
        <p>
          The reasons for this is when a GPU hits a branch, the common behavior
          of GPUs is to run the code for all resulting scenarios, and then
          discard the results of the scenario that wasn't the selected one. This
        ...
        <ul>
          <li>
            ...
            or fragment you're operating on, this shouldn't lead to a
        ...
        <p>
          <em>
            ...
            branching, because either OpenGL optimizes such functions, or the
            GPU has special hardware or driver code optimizations to ensure that
            such code would not have any performance impact. Some of such
            functions (such as <code>clamp</code>) have been supported by GPUs
            before they added support for branching in shader code.
          </em>
        </p>
        <h3>Summary</h3>
        <ul>
          <li>
            Textures are mapped onto objects through a process called UV
            mapping.
          </li>
          <li>
            GPUs perform a similar process by receiving UV texture coordinates
            of vertices, and interpolating them for each fragment, which is then
            used to select the correct pixel color from the texture and use it.
          </li>
          <li>
            Branching is generally discouraged to be performed in shaders and
            can negatively impact performance except for certain scenarios.
          </li>
          <li>
