...
const VertexShaderPage = () => (
  <Layout>
    ...
    <p>
      ...
      understand which vertices are visible to the camera and which are not and
      have to be cut or "clipped" out.
    </p>
    <p>
    ...
    <p>
      Since this shader is executed per vertex on all vertices passed to the GPU
      pipeline, any operation that requires modifications to the vertex can be
      performed during in this shader, as long as the final output is where the
      vertex is to be placed in the clip-space.
    </p>
    <h3>An example - A triangle</h3>
    <p>Below is an example of the work simple vertex shader:</p>
    <VertexShaderFirstExample />
    <p>
      We can see that, for a provided set of vertex positions, a shape is drawn.
      The points on the canvas where the vertices are placed is determined by
      the vertex shader (in part).
    </p>
    <h4>How it works</h4>
    ...
    <p>
      ...
      executed, and should contain the primary shader code to be executed. This
      is similar to C/C++, where the <code>main</code> function in the primary
      entry is the one that is executed when starting the application.
    </p>
    <p>
    ...
    <p>
      ...
      a vector of size 4, but vertices passed to the vertex shader need not be
      limited to just this type, but a <code>vec4</code> type would provide the
      most detail about the coordinates of a vertex.
    </p>
    <p>
    ...
    <dl>
      ...
      <dd>
        ...
        <p>
          ...
          result of where the vertex exists in the world w.r.t. to the center of
      ...
      <dd>
        <p>
          ...
          like field-of-view, aspect ratio, and others can distort and affect
          the way objects look. Likewise, the scale of objects can differ
          depending on distance, which needs to be accounted for.
        </p>
        <p>
          ...
          the camera, taking into account it's aspect ratio, field-of-view, and
          the farthest and closest it can see.
        </p>
        <p>
    ...
    <p>
      This final result is the output of the vertex shader, which, in WebGL, is
      stored in the special variable <code>gl_Position</code>.
    </p>
    ...
    <h4>How it works</h4>
    <p>
      By passing a <code>modelMatrix</code> that's rotated based on the current
      time, the vertex shader will rotate all the vertices by the set amount
    ...
    <p>
      The reason the model matrix rotation calculation is done on the CPU is to
      avoid redundant calculations. Instead of having the vertex shader
      recalculate the same rotation value for every vertex every frame, the CPU
      can calculate it just once every frame and then pass it to the vertex
      shader to apply it.
    </p>
    <p>
      This principle can also be applied to the model, view, and projection
      matrices we multiply to the vertex. By multiplying all these 3 matrices in
      advance on the CPU, and then passing to the vertex shader, we save on
    ...
    <ul>
      <li>
        ...
        onto a space known as the clip-space.
      </li>
      <li>
        The vertex shader requires certain values provided to it about the
        model, view, and projection, in order to be able to determine where the
        final position of the vertex is.
      </li>
      <li>
        Since the shader determines where the vertex is present within this
        space, it can manipulate and transform the vertex to be placed wherever
        required.
      </li>
    </ul>
    <PageChange
